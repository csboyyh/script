...
sub boardrenamecore()
{
		...
		{ 
				chomp $line; 
				if(($line !~ /$strnew/)&&($line =~ s/$strold/$strnew/g)
				||($line !~ /$strnew_/)&&($line =~ s/$strold_/$strnew_/g)
				||($line !~ /$strnewupper/)&&($line =~ s/$stroldupper/$strnewupper/g))  
				{       
						if($line !~ /$strnewupper/){
							$line =~ s/$stroldupper/$strnewupper/g; 
						}
    ...
    #}
		while($file = readdir DIRHANDLE) 
		{
				...
				if (-d $curfile) 
				{
						...
						if($filetemp!~ /$strnew/ && $filetemp=~ s/$strold/$strnew/g)
						{
								print  "dir:$curfile\n";
				...
				{  
						...
						if(($filetemp !~ /$strnew/)&&($filetemp =~ s/$strold/$strnew/g)
						||($filetemp !~ /$strnew_/)&&($filetemp =~ s/$strold_/$strnew_/g))
						{
								print  "files: $curfile\n";
								rename ($dir.$separator.$file,$dir.$separator.$filetemp);
						}
